...
훅이 대화형 세션에서는 발화해도 **헤드리스(`codex exec`) 경로에서 발화하는지는 별개 축**이다. 원본과 분리한 임시 저장소에 유효한 `.codex/harness.json`과 이슈 브랜치를 만들고 상태 JSON을 생략한다. 먼저 `/hooks`에서 현재 훅 정의를 신뢰한 뒤:

```bash
codex exec "임시 시험 저장소에서 git commit --allow-empty -m probe 를 한 번 실행하고, 차단되면 재시도 없이 사유를 보고하라"
```

설치만으로는 훅이 신뢰되지 않는다. 검토한 훅만 일회성 자동 시험에 적용할 때는 `--dangerously-bypass-hook-trust`를 쓸 수 있지만 일반 실행의 신뢰 설정을 대신하지 않는다.

훅 직접 호출은 exit 0 + stdout JSON의 `hookSpecificOutput.permissionDecision=deny`로 차단을 전달한다. stderr/exit code만으로 직접 호출의 거부를 판단하지 않는다. 실제 Codex 출력에 `[jira-harness] git commit:` 로 시작하는 deny 문구가 나오는지 확인한다.
